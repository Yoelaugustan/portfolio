...
const Navbar = ({isDarkMode, setIsDarkMode}) => {
  ...
  return (
    <>
      ...
      <nav className={`w-full fixed px-5 lg:px-8 xl:px-[8%] py-4 flex items-center justify-between z-50 ${isScroll ? "bg-white/50 backdrop-blur-lg shadow-xs dark:bg-dark-theme dark:shadow-white/20" : ""}`}>
        <a href='#home'>
          <Image src={isDarkMode ? "/logo_dark.png" : "/logo.png"} width={90} height={90} alt="" className='cursor-pointer mr-14'/>
        </a>
        ...
        <div className='flex items-center gap-4'>
          ...
          <a href='#footer' className='hidden lg:flex items-center gap-3 px-10 py-2.5 border border-gray-500 rounded-full ml-4 dark:border-white/50'>
        ...
        {/* Mobile Menu */}
        <ul ref={sideMenuRef} className='flex md:hidden flex-col gap-4 py-20 px-10 fixed -right-64 top-0 bottom-0 w-64 z-50 h-screen bg-rose-50 transition duration-500 rounded-tl-4xl rounded-bl-4xl dark:bg-dark-hover dark:text-white'>
          <div className='absolute right-6 top-6' onClick={closeMenu}>
            <Image src={isDarkMode ? assets.close_white : assets.close_black} alt='' className='w-5 cursor-pointer'/>
          </div>
          <li><a className={`${ovo.className}`} onClick={closeMenu} href='#home'>Home</a></li>
